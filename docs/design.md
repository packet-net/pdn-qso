...
- **File**: `FILE-OFFER` (file id, name, size, block size K, CRC-32 of the file); `FILE-SYMBOL` (symbol index, then the LT-coded payload; the receiver regenerates the degree and neighbour set from the index with the agreed PRNG seed); `FILE-STATUS` from the receiver every `statusInterval` (decoded n of K); `FILE-DONE` when decoded and the CRC matches. The sender emits the K source symbols first (systematic), then repair symbols until DONE or a patience limit. Block size is the mode's payload capacity minus the header. The receiver goes on repeating its DONE whenever it hears the sender again, and stops only when it has heard nothing from it for a whole patience (or when somebody offers it another file): a DONE the channel eats leaves the sender pouring symbols at a station that has finished, and what the repeat has to cover is a number of the sender's turns rather than a fixed number of seconds. That was issue #11, and the measurement behind the rule is the ladder in `DoneLingerLadderTests`. **Every answer the receiver makes - its status and its DONE alike, the first one as much as any repeat - waits for the channel to have been quiet for `quietBeforeAnswering` first.** On a half-duplex link the only quiet there is is the gap the sender leaves to listen in, so waiting for quiet is waiting for the gap; answering the instant there is something to say means answering into the sender's own transmission, where neither station hears a word of the other's. That was issue #8, the ladder is `DoneCollisionLadderTests`, and the hold is free because a sender that is transmitting could not have heard the answer any sooner anyway. It gives up after one whole turn of the sender - a status interval of pouring and a listening gap - so a channel somebody else is sitting on cannot silence the receiver altogether.
...
- **A medium that models a collision has to decide it in the protocol's time, not in the
  machine's.** `HalfDuplexChannel` built to collide keeps the queue - two threads inside one
  channel object is a data race, not a collision - and adds what matters: a station is keyed
  from the moment it enters a send until its burst ends, a burst begun with the far end already
  keyed is talked over, and neither station hears the other's frame. Both decisions to key up
  happen at one instant of the clock and on different threads, so before a burst begins the
  medium waits for the far end to either key up or have nothing in hand, using the same `Busy`
  flags the settle loop is given. That costs nothing on the clock, because the clock cannot move
  while either party has work in hand, and it turns "whichever thread won" into a fact about the
  protocol. The same flags are what `IStation.Busy` answers there: the far end's transmitter is
  up while it is between two back-to-back bursts, because this rig computes a burst in
  milliseconds however many seconds of air it is charged, and reading those microsecond gaps
  literally would flatter a receiver waiting for the channel to go quiet by three orders of
  magnitude. Two bursts that overlap on air cost about as much of it as the longer; queued one
  behind the other here they cost the sum, so a collision is a frame time dearer in this rig
  than on air, which errs the safe way.
...
**The one thing the hand-off left open has since been measured and closed.** A receiver's Done collided with the sender's own transmission and was not heard, so a clean transfer cost a status interval more than it should (issue #8). The hermetic tests could not see it, and neither could scaling the intervals down, because the rig's medium queued a station that wanted the channel instead of letting it collide: on it, a receiver answering into the sender's transmission looks exactly like one answering into silence. `HalfDuplexChannel` can now be built to collide - a station keyed up over another loses both frames, and `IStation.Busy` at each end is the other one's transmitter - and on that rig the fault priced out at four times the air time and two and a half times the symbols, and at twenty-one transfers in a hundred and twenty where the file reached disc and the sender never learned.

The issue offered three cures and asked for a measurement rather than a guess, because the answer changes the air time of every transfer. `DoneCollisionLadderTests` is that measurement, at four signal-to-noise ratios over both media, and it carries the table. The receiver now holds an answer until the channel has been quiet for `quietBeforeAnswering`, a quarter of a second by default, which on a half-duplex link means holding it until the sender's listening gap. It is free where nothing was colliding, and the reason it is free is worth keeping: the sender could not have heard the answer any sooner in any case, because it was transmitting.
