...
- **A transfer finishes late.** Less so than it did. The receiver used to answer the instant it had the file, which over a half-duplex link is the instant the sender is transmitting, so its Done went into the sender's own transmission and the sender only learned on its next turn (issue 8). It now holds an answer until the channel has been quiet for a quarter of a second, which puts it in the gap the sender leaves to listen in; on the channel rig that took a clean transfer from twenty-nine seconds of air to thirteen and cost nothing where nothing was colliding. What is left is the honest part: the receiver reports on a status interval, so a small file can be on disc for a fraction of one before the sender stops. The file is intact either way.
